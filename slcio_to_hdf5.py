...
def fill_calohit_datasets(slcio_coll, hdf5_coll):
    ...
    """
    hit_times = np.array([0])
    hit_energy = np.array([0])
    hit_cell_id = np.array([0])
    hit_x_pos = np.array([0])
    hit_y_pos = np.array([0])
    hit_z_pos = np.array([0])

    for hit in slcio_coll:
        hit_times = np.append(hit_times, hit.getTime())
        hit_energy = np.append(hit_energy, hit.getEnergy())
        hit_cell_id = np.append(hit_cell_id, hit.getCellID0())
        hit_x_pos = np.append(hit_x_pos, hit.getPosition()[0])
        hit_y_pos = np.append(hit_y_pos, hit.getPosition()[1])
        hit_z_pos = np.append(hit_z_pos, hit.getPosition()[2])

    hit_sets = {"hit_times": hit_times, "hit_energy": hit_energy, "hit_cell_id": hit_cell_id, 
                "hit_x_pos": hit_x_pos, "hit_y_pos": hit_y_pos, "hit_z_pos": hit_z_pos}
...
def fill_trackerhit_datasets(slcio_coll, hdf5_coll):
    ...
    for hit in slcio_coll:
        hit_times = np.append(hit_times, hit.getTime())
        hit_e_dep = np.append(hit_e_dep, hit.getEDep())    
        hit_cell_id = np.append(hit_cell_id, hit.getCellID0())
        hit_x_pos = np.append(hit_x_pos, hit.getPosition()[0])
        hit_y_pos = np.append(hit_y_pos, hit.getPosition()[1])
        hit_z_pos = np.append(hit_z_pos, hit.getPosition()[2])

    hit_sets = {"hit_times": hit_times, "hit_e_dep": hit_e_dep, "hit_cell_id": hit_cell_id, 
                "hit_x_pos": hit_x_pos, "hit_y_pos": hit_y_pos, "hit_z_pos": hit_z_pos}
